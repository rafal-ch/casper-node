...
impl FromCapnpReader<BlockHeader> for block_header_capnp::block_header::Reader<'_> {
    fn try_from_reader(&self) -> Result<BlockHeader, DeserializeError> {
        ...
        Ok(BlockHeader::new(
            ...
            height,
            protocol_version,
            200, // Here we need to "convert" the deserialized block_header to V2 and provide values for the added fields.
        ))
    }
}
...
pub(crate) mod tests {
    ...
    #[test]
    fn demo_1_serialize_current_block_header_to_file() {
        let block_header = random_block_header(None);
        let serialized = block_header.try_to_capnp_bytes().expect("serialization");
        let hex_encoded = hex::encode(serialized);
        fs::write("/tmp/old_block_header.bin", hex_encoded).expect("can't write");
    }

    #[test]
    fn demo_2_deserialize_updated_block_header_from_file() {
        let bytes = fs::read("/tmp/old_block_header.bin").expect("can't read");
        let hex_decoded = hex::decode(bytes).expect("decode");
        let bytes_slice = hex_decoded.as_slice();
        let deserialized =
            capnp::serialize::read_message(bytes_slice, capnp::message::ReaderOptions::new())
                .expect("read message");
        let reader = deserialized
            .get_root::<block_header_capnp::block_header::Reader>()
            .expect("get reader");

        let new_id = reader.get_new_id(); // Note: No `has_new_id()` generated by `capnp`.
        println!("'new id' of the extended structure: {}", new_id);

        let deserialized =
            BlockHeader::try_from_capnp_bytes(&bytes_slice).expect("deserialization");
        println!(
            "'new id' of the extended structure: {}",
            deserialized.new_id()
        );
    }
}
