...
impl ChainspecLoader {
    ...
    {
        error!("XXXXX - handle_initialize() START");
    ...
    fn new_upgrade_config(
        ...
        error!("XXXXX - new_upgrade_config() START");
        ...
        error!("XXXXX - new_upgrade_config() END");
    ...
    where
        REv: From<ContractRuntimeRequest>
            ...
            + Send,
    {
        error!("XXXXX - handle_commit_genesis_result() START");
        match result {
            Ok(GenesisSuccess {
                post_state_hash, ..
            }) => {
                ...
                let x = self.execute_immediate_switch_block(
                    effect_builder,
                    None,
                    initial_pre_state,
                    finalized_block,
                );
                error!("XXXXX - handle_commit_genesis_result() END1");
                x
            }
            Err(error) => {
                error!(%error, "failed to commit genesis");
                error!("XXXXX - handle_commit_genesis_result() END2");
    ...
    where
        REv: From<ContractRuntimeRequest>
            ...
            + Send,
    {
        error!("XXXXX - handle_upgrade_result() START");
        match result {
            Ok(UpgradeSuccess {
                post_state_hash, ..
            }) => {
                ...
                error!("XXXXX - handle_upgrade_result() END1");
                x
            }
            Err(error) => {
                error!("failed to upgrade contract runtime: {}", error);
                error!("XXXXX - handle_upgrade_result() END2");
    ...
    where
        REv: From<ContractRuntimeRequest>
            ...
            + Send,
    {
        error!("XXXXX - execute_immediate_switch_block() START");
        ...
        async move {
            ...
            error!("XXXXX - execute_immediate_switch_block() END");
    ...
    {
        error!("XXXXX - handle_execute_immediate_switch_block_result() START");
        ...
        match maybe_era_end {
            ...
            None => {
                error!("XXXXX - handle_execute_immediate_switch_block_result() END1");
        ...
        self.reactor_exit = Some(ReactorExit::ProcessShouldContinue);
        error!("XXXXX - handle_execute_immediate_switch_block_result() END2");
