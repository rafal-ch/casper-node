...
use casper_types::{
    binary_port::{
        ...
        binary_response::{BinaryResponse, PayloadType},
        db_id::DbId,
        get::GetRequest,
    ...
    FinalizedApprovals, FinalizedDeployApprovals, Key, KeyTag, NextUpgrade, PeersMap,
...
pub trait NodeClient: Send + Sync {
    ...
    async fn read_genesis_account_bytes(&self) -> Result<Option<Vec<u8>>, Error> {
        let resp = self
            .read_from_mem(NonPersistedDataRequest::GenesisAccountsBytes)
            .await?;
    ...
    async fn read_global_state_bytes(&self) -> Result<Option<Vec<u8>>, Error> {
        let resp = self
            .read_from_mem(NonPersistedDataRequest::GlobalStateBytes)
            .await?;
        parse_response::<Vec<u8>>(&resp)
    }
...
impl PayloadEntity for PeersMap {
    const PAYLOAD_TYPE: PayloadType = PayloadType::PeersMap;
}
...
impl PayloadEntity for bool {
    const PAYLOAD_TYPE: PayloadType = PayloadType::Bool;
}
