...
mod test {
    ...
    fn first_hash_in_proof_matches_data_hash() {
        let large_data_size = (ChunkWithProof::CHUNK_SIZE * 1024) as usize;
        let large_data = prepare_bytes(large_data_size);

        let number_of_chunks: u64 = large_data
        ...
        (0..number_of_chunks)
            .into_iter()
            .map(|chunk_index| ChunkWithProof::new(large_data.as_slice(), chunk_index).unwrap())
    ...
    fn validate_chunks_against_hash_merkle_tree() {
        let data = prepare_bytes(ChunkWithProof::CHUNK_SIZE * 2);
        let expected_root =
